...
run_solana_bootstrap() {
  ...
  declare rpc_port=$4
  solana-validator --enable-rpc-exit --enable-rpc-set-log-filter --gossip-host $host --ledger $datadir --dynamic-port-range $port_range --rpc-port $rpc_port --snapshot-interval-slots 200 --identity $datadir/identity.json --vote-account $datadir/vote-account.json --log -
}
